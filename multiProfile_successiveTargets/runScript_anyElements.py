...
#####         setup the symmetry for forward foil
        ...
        ##### center
        for j in range(pts0.shape[1]):
            shapes0.append({pts0[int(pts0.shape[0]/2), j, 0]: dir_y, pts0[int(pts0.shape[0]/2), j, 1]: dir_y})
        
        
        ############################
        ##### LE TE
        shapes0.append({pts0[0, 0, 0]: dir_x, pts0[-1, 0, 0]: -dir_x, pts0[0, 0, 1]: dir_x, pts0[-1, 0, 1]: -dir_x, pts0[0, 1, 0]: -dir_x, pts0[-1, 1, 0]: dir_x, pts0[0, 1, 1]: -dir_x, pts0[-1, 1, 1]: dir_x})
...
#            self.add_constraint("geometry.linearcon2"+str(ii), equals=0.0, scaler=1.0, linear=True)
        ...
        self.add_constraint("geometry.thickcon0", lower=1., upper=3.0, scaler=1)
